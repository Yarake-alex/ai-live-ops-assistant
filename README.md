...
AI 直播运营助手 MVP 是一套面向直播电商运营场景的 AI 演示系统，围绕「商品资料 → 资料问答 → 开播话术 → 评论回复 → 直播复盘 → 运营建议」形成完整的轻量业务闭环。

系统基于 Python、FastAPI、SQLAlchemy、SQLite / PostgreSQL、Vue3 + Vite 与 OpenAI-compatible API 开发，支持商品资料管理、商品资料文档 RAG 问答、直播素材库、问题记录、问题洞察、资料完整度评分、运营建议、评论助手、直播话术生成、直播复盘、登录鉴权、管理员用户管理、CSV 导入导出、Docker / PostgreSQL 部署与自动化测试。未配置真实 LLM 或 Embedding 时，系统会自动降级到 mock、TF-IDF 或本地规则能力，保证演示流程不断链。

当前业务主线为 **V4：问题驱动的资料补齐与话术优化**。系统会把直播间常见问题沉淀为问题日志和洞察结果，再转成可执行的资料补齐、FAQ 候选、话术强化与风险提醒建议，帮助运营人员知道「用户在问什么、资料缺什么、下场直播应该怎么讲」。

旧版 CRM 客户管理 / 跟进分析 / 待跟进 / Agent 跟进助手相关能力已下线，仅保留 Git 历史记录。

## 核心功能

1. **商品资料管理**：支持商品新增、查看、编辑、删除、搜索、筛选、分页、CSV 导入导出，覆盖价格、卖点、人群、痛点、优惠、库存、直播状态等字段。
2. **商品资料文档问答**：按商品上传 PDF / TXT / MD / CSV 资料，支持文档列表、片段查看、重新整理资料、删除文档和基于当前商品资料的问答。
3. **直播素材库**：提供独立于商品维度的通用资料库，支持资料上传、搜索、片段预览、重新整理、清空和资料问答。
4. **资料完整度评分**：按固定规则实时计算商品开播准备完整度，展示缺失项和下一步建议，帮助运营优先补齐关键资料。
5. **问题记录与本地快答**：记录商品问答和评论输入；价格、库存、优惠、适用人群、核心卖点等低风险问题优先走本地规则回答。
6. **问题洞察与运营建议**：统计高频问题、分类分布、最近问题和未覆盖问题，并生成资料补齐、FAQ 候选、话术强化、风险提醒等运营动作建议。
7. **直播话术生成**：结合商品资料、高频问题、未覆盖问题和风险问题生成主播口播话术，AI 不可用时自动使用本地兜底话术。
8. **评论助手**：根据商品信息和观众评论生成主播口吻回复，资料未覆盖的内容不编造，避免绝对化承诺。
9. **直播复盘**：基于商品资料、评论样本和历史回复生成复盘，覆盖用户关注点、常见异议、高频问题和下场直播优化建议。
10. **账号与部署能力**：支持访问密码登录、HttpOnly Cookie 登录态、管理员用户管理、用户启用 / 禁用、Docker 部署、PostgreSQL 生产连接和备份恢复说明。

## 技术关键词

Python、FastAPI、SQLAlchemy、SQLite、PostgreSQL、Vue3、Vite、HTML、CSS、JavaScript、Pydantic、OpenAI-compatible API、DeepSeek API、Prompt Engineering、RAG、TF-IDF、Embedding、ChromaDB、pypdf、scikit-learn、AI Agent、Docker、Linux、OpenResty、pytest、Cookie Session、用户隔离、CSV 导入导出、备份恢复。

## 项目亮点

1. **业务闭环完整**：从商品录入、资料上传、资料问答、开播话术、评论回复到复盘和运营建议，贴近真实直播运营流程。
2. **工程化程度较高**：包含鉴权、用户隔离、数据库升级、测试、Docker 部署、健康检查、PostgreSQL 部署和备份恢复文档。
3. **AI 能力可控**：真实 API、mock、本地规则和检索降级并存，超时、失败或未配置时仍能完成演示。
4. **部署成本轻**：默认 SQLite 可直接本地运行，生产可切换 PostgreSQL；Embedding / Chroma 可选启用，适合小型云服务器演示。
5. **前端工作台化**：Vue3 前端已承载主要演示入口，围绕商品资料、素材库、评论助手、运营工作台和用户管理形成后台式布局。

## V5 UI 设计状态

V5 前端已从旧 `static/index.html` 迁移到 Vue3 + Vite，并作为默认演示入口使用；旧页面仍可通过 `/legacy` 与 `/static/index.html` 作为回退入口访问。

目前 **V5 的 UI 设计正在重构和重新设计，暂未完成**。当前重点是统一视觉规范、页面布局、组件结构、交互反馈、响应式表现和可访问性，让系统从「功能可演示」进一步提升为「更适合正式展示的轻量运营后台」。业务功能、接口和数据库仍沿用现有 V4 主线能力。
...
- **运营建议**：`GET /products/{id}/ops-suggestions` 基于问题日志、问题洞察、资料完整度与商品资料状态生成确定性运营建议（资料补齐 / FAQ 候选 / 话术强化 / 风险提醒，规则优先不调模型）；前端「问题洞察」Tab 顶部「运营建议」卡片轻量展示。
- **直播话术**：基于商品资料生成七模块主播口播话术（开场引入 → 结尾转化），并参考高频问题、未覆盖问题与风险问题自动追加「直播间常问应答」模块；AI 不可用时自动降级为本地兜底话术并诚实标注资料缺失项。
...
- **前端**：Vue3 + Vite（V5 UI 设计重构中，暂未完成）；旧版 HTML/CSS/JavaScript 单页保留为回退入口
...
frontend/               # Vue3 + Vite 前端（V5 UI 设计重构中，npm run build 输出到 frontend/dist）
├── src/                # 组件与视图源码
├── index.html
└── vite.config.js
...
| `/products/{id}/ops-suggestions` | GET | 商品级运营建议（资料补齐/FAQ/话术/风险提醒） | 是 |
...
- 运营建议规则（资料补齐/FAQ 候选/话术强化/风险提醒、空数据、隔离、零模型调用）；
- 话术常问应答（高频问题注入、风险不承诺、兜底增强、上下文隔离）；
- 直播话术、评论助手、直播复盘与本地兜底；
- 运营工作台字段、计数与用户隔离；
- 未登录请求业务接口返回 401；
- 错误密码登录返回 401；
...
| V3 | 问题洞察 + 本地快答 | 商品问题记录与分类、低风险本地规则快答、商品级问题洞察接口与前端面板 |
| V4 | 问题驱动的资料补齐与话术优化（当前） | 运营建议（资料补齐/FAQ 候选/话术强化/风险提醒）、话术常问应答增强 |

## 下一步建议

- 根据真实演示反馈优化页面信息密度与交互细节；
- 将高频/未覆盖问题进一步联动到直播复盘输入，形成"提问 → 建议 → 内容"的完整闭环；
- 探索建议闭环：运营按建议补资料后，让建议状态自动反馈（轻量方向，不设计复杂系统）。
