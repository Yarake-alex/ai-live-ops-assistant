...
## V5 进行中：前端 Vue3 组件化重构

> 说明：当前版本仍是直播运营 V4（V1 直播运营 MVP → V2 资料检索增强 + 开播准备 → V3 问题洞察 + 本地快答 → V4 问题驱动的资料补齐与话术优化）。V5 正在按阶段迁移中，**不代表 V5 全部完成**。
...
- 阶段 5.4：用户管理已迁移到 Vue3——新增用户、用户列表、启用 / 禁用、刷新。
- **旧 `static/index.html` 仍保留，当前正式演示流程不受影响**；Vue3 前端（`frontend/`）是迁移中的新前端，仅作逐页迁移的载体。

### 验证状态

- 每个前端阶段均通过 `npm run build` 构建检查。
- 已做本地烟测：后端 uvicorn + Vite dev 代理链路（页面服务、`/auth`、`/rag`、`/products`、`/live-scripts`、`/live-reviews` 转发、未登录 401 处理路径）。
- 真实数据的完整联调受本地登录配置限制（本机 `.env` 密码非演示默认值，按约束不读取 `.env`），待后续阶段在可登录环境补充。

### 下一步建议

1. 收尾剩余能力：商品删除、修改密码（页头登录态功能）等未迁移项。
2. 全部迁移完成后，评估以 Vue3 前端作为正式演示入口并下线旧单页（保留 Git 历史）。
3. 每完成一页迁移即跑全量 pytest 回归，确认接口与业务行为不变。
