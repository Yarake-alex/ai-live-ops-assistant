...
V1-V7 是整个仓库/项目的历史演进编号：V1-V4 是历史旧路线与工程基础阶段，V5-V7 是当前直播运营路线。

| 阶段 | 版本 | 能力 | 当前状态 |
|---|---|---|---|
| 历史旧路线 | V1 | 客户管理 + AI 跟进建议 | 已下线，仅保留 Git 历史 |
| 历史旧路线 | V2 | 通用资料问答 | 已演进为直播素材库，作为辅助能力保留 |
| 历史旧路线 | V3 | Agent 跟进助手 | 已下线，仅保留 Git 历史 |
| 工程基础 | V4 | 企业增强版 | 安全加固、数据库工程化、测试体系、Docker 部署等工程能力保留 |
| 当前直播运营路线 | V5 | 直播运营 MVP | 商品资料文档、开播话术、评论助手、直播复盘、运营工作台 |
| 当前直播运营路线 | V6 | 资料检索增强 + 开播准备 | 商品资料语义检索、资料完整度评分、开播准备入口 |
| 当前直播运营路线 | V7 | 问题洞察 + 本地快答 | **当前版本**；问题记录、低风险本地快答、商品级问题洞察面板 |
...
本项目为 AI 直播运营助手 MVP，当前版本为 **V7（问题洞察 + 本地快答）**，演示链路覆盖「商品资料 → 资料文档问答 → 开播话术 → 评论助手 → 直播复盘 → 运营工作台 → 资料完整度 → 问题洞察」，同时保留了安全、测试、配置、数据库和 Docker 部署等工程实践内容，适合作为 AI 应用工程师方向的综合项目展示。旧版 CRM 客户管理 / 跟进分析 / 待跟进 / Agent 跟进助手相关代码、页面与测试已在 `chore: remove legacy CRM` 系列提交中移除，历史实现可查看 Git 历史。
