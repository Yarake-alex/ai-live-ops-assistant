...
## V5 计划：前端 Vue3 组件化重构（准备启动）

> 说明：当前版本仍是直播运营 V4（V1 直播运营 MVP → V2 资料检索增强 + 开播准备 → V3 问题洞察 + 本地快答 → V4 问题驱动的资料补齐与话术优化）。V5 尚未启动开发，本节仅记录计划与边界。
...
1. 先搭 Vue3 最小骨架（与现有静态页面并存，接口行为不变）。
2. 再逐页迁移（商品资料 → 直播素材库 → 评论助手 → 其他页面），每一步保持可运行、可回退。
