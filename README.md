...
- 阶段 3：直播素材库已迁移到 Vue3 并形成闭环——列表展示、文件名搜索、上传确认（同批重复拦截 / 同名覆盖提醒 / 逐个上传）、片段预览（原地预览 / 收起）、删除、清空、重新整理、资料问答（参考资料折叠）。
- 阶段 4：商品资料页已迁移到 Vue3——**商品选择、当前商品基础信息、资料完整度、资料文档列表、资料问答（本地快答 / 知识库问答）、问题洞察、运营建议、直播话术、直播复盘**。
- 阶段 5：素材库资料问答与评论助手已迁移到 Vue3——评论助手包含商品选择、快捷问题、生成回复、结果展示、历史列表、查看详情。
...
1. 继续迁移剩余页面与能力：运营工作台、商品新增/编辑/CSV 导入导出、用户管理等辅助页面。
2. 每一步保持可运行、可回退，旧页面在全部迁移完成前继续作为正式演示入口。
