...
当前主线能力围绕直播运营人员的日常工作展开：商品资料维护、商品资料文档问答（语义检索 + 基础检索降级）、开播话术、评论助手、直播复盘、运营工作台、资料完整度评分、开播准备入口，以及问题记录、低风险本地快答、商品级问题洞察和**运营建议**（资料补齐 / FAQ 建议 / 话术强化 / 风险提醒）。系统把"开播前 → 直播中 → 下播后"的流程串成一条可演示的轻量工作流，并把"用户常问什么"转成"该补什么资料、该讲什么话"。
...
## V5：前端 Vue3 组件化重构（迁移完成，UI 设计重构中）
...
> UI 状态：V5 的 UI 设计正在重构和重新设计，暂未完成；当前业务功能仍沿用直播运营 V4 主线能力。

### 为什么要做

- `static/index.html` 已经承载过多页面结构、状态和交互逻辑（商品资料、直播素材库、评论助手、问题洞察、运营建议、话术生成、复盘等），继续在单文件里堆功能会增加维护成本。
- Vue3 重构用于组件拆分、状态清晰、后续 UI 统一，**不改变业务接口**。

### 当前进度

- 阶段 0：V5 基线备份（tag `pre-v5-vue3`、分支 `backup/pre-v5-vue3`）与 README 计划说明。
- 阶段 1：`frontend/` Vue3 + Vite 最小骨架（vue / vite / @vitejs/plugin-vue，无其他依赖）。
- 阶段 2：基础层——API client（统一 fetch/JSON/错误处理）、会话状态（Vue reactive）、AppShell 布局壳。
- 阶段 3：直播素材库已迁移到 Vue3 并形成闭环——列表展示、文件名搜索、上传确认（同批重复拦截 / 同名覆盖提醒 / 逐个上传）、片段预览（原地预览 / 收起）、删除、清空、重新整理、**资料问答（参考资料折叠，归入本阶段闭环）**。
- 阶段 4：商品资料页已迁移到 Vue3——**商品选择、当前商品基础信息、资料完整度、资料文档列表、资料问答（本地快答 / 知识库问答）、问题洞察、运营建议、直播话术、直播复盘**。
- 阶段 5.1：评论助手已迁移到 Vue3——商品选择、快捷问题、生成回复、结果展示、历史列表、查看详情。
- 阶段 5.2：运营工作台已迁移到 Vue3——运营概览统计（商品数 / 直播中商品 / 话术 / 评论回复 / 复盘 / 资料文档）、开播准备横幅、直播流程快捷入口。
- 阶段 5.3a：商品新增与编辑已迁移到 Vue3——列表「新增商品」、详情「编辑商品」、模态表单、前端校验、保存后刷新列表与详情。
- 阶段 5.3b：商品 CSV 导入导出已迁移到 Vue3——导入汇总提示（创建 / 跳过 / 错误明细）、导出下载、表头说明。
- 阶段 5.4：用户管理已迁移到 Vue3——新增用户、用户列表、启用 / 禁用、刷新（入口仅管理员可见，与旧页面一致）。
- 阶段 5.5：剩余小功能已迁移——商品删除（确认 / 清选中 / 刷新）、修改密码（成功后自动退出登录）、退出登录。
- 阶段 5.6：登录流程已迁移——登录页（空密码 / 失败 / 登录中状态）、会话恢复、401 自动回到登录页。
- 阶段 5.7：补齐功能对照发现的缺口——商品资料文档上传（确认弹窗）、片段预览、文档删除、重新整理资料、开播准备概览条。
- 阶段 6：功能对照与全量验证——两次对照复核（6.1 / 6.2），缺口已全部闭合。

### 需确认的设计取舍（记录在案，未修改）

1. **商品资料页布局**：旧页面使用 Tabs 分区，Vue 前端为纵向堆叠卡片——功能覆盖一致，交互形态不同；V5 UI 设计仍在重构和重新设计中。
2. **跨页联动**：旧页面评论助手生成回复后会同步刷新商品资料页的问题洞察 / 运营建议（依赖全局选中状态）；Vue 中两页为独立视图，无该联动——属可选优化。

### 验证状态

- 每个前端阶段均通过 `npm run build` 构建检查；最终构建产物正常。
- 后端全量 pytest 通过：**389 passed, 1 warning**（既有 fixture 弃用警告，与前端无关）。
...
本项目为 AI 直播运营助手 MVP，当前版本为 **直播运营 V4（问题驱动的资料补齐与话术优化）**，演示链路覆盖「商品资料 → 资料文档问答 → 开播话术 → 评论助手 → 直播复盘 → 运营工作台 → 资料完整度 → 问题洞察 → 运营建议」，同时保留了安全、测试、配置、数据库和 Docker 部署等工程实践内容，适合作为 AI 应用工程师方向的综合项目展示。旧版 CRM 客户管理 / 跟进分析 / 待跟进 / Agent 跟进助手相关代码、页面与测试已在 `chore: remove legacy CRM` 系列提交中移除，历史实现可查看 Git 历史。
