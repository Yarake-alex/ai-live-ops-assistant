...
- 阶段 3：直播素材库已迁移到 Vue3——**列表展示、文件名搜索、上传确认（同批重复拦截 / 同名覆盖提醒 / 逐个上传）、片段预览（原地预览 / 收起）**；素材库的删除、清空、重新整理等能力仍待后续阶段迁移。
- **旧 `static/index.html` 仍保留，当前正式演示流程不受影响**；Vue3 前端（`frontend/`）是迁移中的新前端，仅作逐页迁移的载体。

### 验证状态

- 每个前端阶段均通过 `npm run build` 构建检查。
- 已做本地烟测：后端 uvicorn + Vite dev 代理链路（页面服务、`/auth`、`/rag` 转发、未登录 401 处理路径）。
- 真实数据的完整联调受本地登录配置限制（本机 `.env` 密码非演示默认值，按约束不读取 `.env`），待后续阶段在可登录环境补充。

### 下一步建议

1. 继续迁移直播素材库剩余能力（删除、清空、重新整理）与资料问答。
2. 再逐页迁移（商品资料 → 评论助手 → 其他页面），每一步保持可运行、可回退。
