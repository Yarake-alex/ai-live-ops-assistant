...
|---|---|---|
| 1 | V4 设计说明 | 本文档（**当前阶段**） |
| 2 | 后端建议生成 helper + `ops-suggestions` 接口 | `app/question_insights.py`、`app/schemas.py`、`app/main.py` |
| 3 | 测试 | `tests/test_ops_suggestions.py`（规则/隔离/空数据 + 全量回归） |
| 4 | 前端运营建议卡片 | `static/index.html`（问题洞察 Tab 内轻量卡片） |
| 5 | 话术生成增强 | 话术 prompt 附加「直播间常问应答」（响应结构不变，兼容前端） |
| 6 | 文档同步 | README、demo-guide、mvp-scope 同步 V4 说明 |
