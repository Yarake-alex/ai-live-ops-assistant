...
| 向量检索改造 | 商品资料文档固定使用 TF-IDF；直播素材库页支持向量检索但默认关闭（`VECTOR_SEARCH_ENABLED=false`） |
...
1. **商品资料文档「重建该文件索引」为诚实 no-op**：TF-IDF 每次即时计算、无索引可重建，
...
1. **向量检索升级**：接入 Embedding API + ChromaDB 替换/混合 TF-IDF，提升语义召回；
   同时让商品资料文档的上传/重建索引接入向量写入，使「重建该文件索引」具备真实语义。
