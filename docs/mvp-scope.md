...
- 12 项固定权重规则（总分 100）：名称非空、`price > 0`、`stock > 0`、直播状态、核心卖点、适用人群、用户痛点、优惠信息、商品资料文档存在、FAQ 类文件名（`faq` / `问答` / `Q&A`）、售后类文件名（`售后` / `退换` / `after`）、风险边界（`notes` 非空或文件名含 `风险` / `边界` / `禁用` / `不可承诺` / `合规`）。
- `score = 命中维度权重求和`（0-100 整数）；`missing_items` 与 `suggestions` 均为确定性映射的运营语言文案。
- 评分为固定权重规则：名称/价格/库存/直播状态各 5 分，核心卖点/适用人群/商品资料文档/FAQ 各 10 分，用户痛点 8 分、优惠信息 7 分，售后规则 12 分、风险边界 13 分——风险、售后、FAQ 等对开播准备价值更高的维度权重更大，用于演示“缺什么补什么”，不代表 AI 对资料质量做主观判断。
...
- 当前验证结果：全量 377 个用例通过、1 个既有 fixture 弃用 warning（`tests/test_question_insights.py` 与 `tests/test_live_scripts.py` 覆盖 V4 相关规则与隔离）。
...
### 11. 问题驱动的资料补齐与话术优化（V4）
- **运营建议接口**：`GET /products/{id}/ops-suggestions`，基于问题日志、问题洞察、资料完整度与商品资料状态
  生成确定性建议（**规则优先，不调用额外模型、不走向量、不新增表字段**），按 `user_id + product_id` 隔离。
- **建议类型**：`material_gap`（资料补齐）/ `faq_candidate`（FAQ 候选，只建议不写文档）/
  `script_focus`（话术强化）/ `risk_reminder`（风险提醒）；priority high/medium/low；同类去重、最多 8 条。
- **前端运营建议卡片**：商品资料页「问题洞察」Tab 顶部「💡 运营建议」卡片（建议总数 / 高优先级 / 资料状态 +
  建议列表含来源问题与动作标签）；空态与失败态温和，不影响其他功能。
- **话术常问应答增强**：生成直播话术时注入当前商品的高频问题、未覆盖问题、低风险本地快答问题与风险问题；
  有问题数据时生成「八、直播间常问应答」；未覆盖问题提示"资料暂未明确，建议补充后再承诺"，风险问题不做
  医疗/功效/绝对化承诺；AI 不可用时本地兜底话术同样追加该模块；无问题数据时不硬编；上下文生成 best-effort，
  失败不影响话术生成。
...
| 自动修改商品资料 | 运营建议只提示方向，不自动改写任何商品字段 |
| 自动生成并保存 FAQ 文档 | FAQ 建议仅提示"建议新增 FAQ"，由运营手动沉淀 |
| 语音识别 | 问题输入为文本，不做语音转写 |
